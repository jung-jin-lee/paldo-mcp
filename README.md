...
- **모델**: `Xenova/multilingual-e5-small` (384d, 약 120MB ONNX, 한국어 강함)
- **저장**: `~/.paldo/index/embeddings.parquet` (~1.5GB)
- **소요시간 (Apple Silicon CPU 기준)**: 16-20시간 (~16 rows/sec). 빌드 도중 중단되어도 5,000행마다 체크포인트가 저장되므로 같은 명령을 다시 실행하면 이어서 진행합니다.
